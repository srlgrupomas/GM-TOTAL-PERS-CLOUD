report 34006879 "PersCredit Note (E) - Sales "
{
        ...
        dataitem("Sales Invoice Header"; "Sales Cr.Memo Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column("SalesInvoiceHeader___VAT_Registration_No__"; "Sales Invoice Header"."VAT Registration No.")
                    {
                    }
                    ...
                    column(InfoEmpresa_Address; InfoEmpresa.BssiBillingAddr1 + ', ' + InfoEmpresa.BssiBillingZipCode + ', ' + gProvincia + ', ' + recPais.name)
                    {
                    }
                    ...
                    column(DocNUMAF; "Sales Invoice Header"."GMAAFIP Voucher Type")
                    {
                    }
                    ...
                    column(GMACUITExtranjero; "GMACUITExtranjero")
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Cr.Memo Line")
                    {
                        ...
                        begin
                            ...
                            gDescuento := 0;
                            VatDif := "Sales Invoice Line"."Inv. Discount Amount" * ("Sales Invoice Line"."VAT %" / 100);
                            ...
                            NoRemito := RecItemLed."Document No.";
            ...
            begin
                ...
                RecReturnReceiptHeader.SETCURRENTKEY("No.");
                RecReturnReceiptHeader.SETRANGE("Applies-to Doc. No.", "Sales Invoice Header"."Applies-to Doc. No.");
                RecReturnReceiptHeader.SETRANGE("Applies-to Doc. Type", "Sales Invoice Header"."Applies-to Doc. Type");
                ...
                if "Sales Invoice Header"."Currency Code" <> '' then begin
                    ...
                    ImporteLetras := 'Son ' + Moneda.Description + ': ' + NumberText[1] + NumberText[2] + '  a TC: ' + FORMAT(ROUND(1 / factor));
                end
                else
                    ...
                    repeat
                        case tmoviva."GMATax Type Loc" of
                            ...
                            tmoviva."GMATax Type Loc"::"IVA Percepcion":
                                ImporIVA2 := ImporIVA2 + (tmoviva.Amount * factor);
                            tmoviva."GMATax Type Loc"::"Ingresos Brutos":
                                ImporIVA3 := ImporIVA3 + (tmoviva.Amount * factor);
                            tmoviva."GMATax Type Loc"::Otros:
                                ImporIVA4 := ImporIVA4 + (tmoviva.Amount * factor);
                        end;
                ...
                // AW - BEGIN
                ImporteBase2 := ROUND(ImporteBase1, 0.01);
                // AW - END
                ...
                if Rec18.FINDFIRST then
                    gTel := Rec18."Phone No.";
                GMACUITExtranjero := Rec18.GMACUITExtranjero;
                ...
                _i := 0;
                _recVatEntry.RESET;
                _recVatEntry.SETCURRENTKEY("Entry No.");
                _recVatEntry.SETRANGE("Document No.","Sales Invoice Header"."No.");
                _recVatEntry.SETFILTER("Tipo de Impuesto", '<>%1' , _recVatEntry."Tipo de Impuesto"::IVA);
                IF _recVatEntry.FINDSET THEN REPEAT
                  _recTaxJur.RESET;
                  ...
                  ELSE
                   ...
                   MatrizIIBBPer[_i,2] := _DescripJuris + '    Total: ' + "Sales Invoice Header"."Currency Code" + ' ' +
                                          FORMAT(ABS(_recVatEntry.Amount));
                  IF _recVatEntry."Tipo de Impuesto" = _recVatEntry."Tipo de Impuesto"::"Ingresos Brutos" THEN BEGIN
                    TotalIvaPerIBB += ABS(_recVatEntry.Amount);
                    TotalBase += _recVatEntry.Base;
                  END ELSE BEGIN
                    TotalIvaPer += ABS(_recVatEntry.Amount);
                    TotalBaseIVAPer := _recVatEntry.Base;
                  END;
                UNTIL _recVatEntry.NEXT = 0;
                TotalBase := ROUND(TotalBase,0.01);
                IF TotalBase <> 0 THEN
                  porcentajeIVAPerIIBB := ABS(ROUND((TotalIvaPerIBB / TotalBase) * 100, 0.01));
                
                TotalBaseIVAPer := ROUND(TotalBaseIVAPer,0.01);
                IF TotalBaseIVAPer <> 0 THEN
                  porcentajeIVAPer := ABS(ROUND((TotalIvaPer / TotalBaseIVAPer) * 100,0.01));
                //NAVAR1.06004-
                */
                //NAVAR1.06003-
                _i := 0;
    ...
    var
        ...
        MatrizIIBBPer: array[10, 2] of Text[250];
        TotalBase: Decimal;
